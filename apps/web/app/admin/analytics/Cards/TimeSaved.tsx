...
export async function TimeSaved({ start, end }: { start: Date, end: Date }) {
    const { sessionClaims } = auth()
    const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/v1/admin/count-ai-tickets`, {
    ...
    return (
        <CardDataStats title="Time Saved" total={`${(count * 6.3).toFixed(1)} minutes`}>
            <svg
                version="1.1"
                id="Layer_1"
